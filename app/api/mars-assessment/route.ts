...
export async function POST(req: Request) {
  ...
  if (
    ...
    formData.medicalCondition === undefined
  ) {
...
Be detailed, professional, and specific in your analysis.`;
    ...
    const survivalChance = `Internal Assessment: Your estimated survival chance is ${baseSurvival}% - ${
      baseSurvival + 5
    }%. (Note: Calculated using simple internal risk factors due to AI failure).`;
    ...
    if (sleep < 7)
      improvements += `**Improve sleep routine (currently ${sleep}h).** A minimum of 7-8 hours is essential for cognitive function and stress resilience.`;
